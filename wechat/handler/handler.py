...
#             return parser_text(xml, text)
        # 针对段子特殊处理
        if msg_content == '段子' or msg_content == '来个段子':
        ...
        return handle_image(xml)
    elif msg_type == 'voice':
        return handle_voice(xml)
    elif msg_type == 'video':
        return handle_video(xml)
    elif msg_type == 'shortvideo':
        return handle_shortvideo(xml)
    elif msg_type == 'location':
        return handle_location(xml)
    else:
        return 'success'

def handle_image(xml):
    return parser_text(xml, text='got img')

def handle_voice(xml):
    return parser_text(xml, text='got voice')

def handle_video(xml):
    return parser_text(xml, text='got video')

def handle_shortvideo(xml):
    return parser_text(xml, text='got shortvideo')

def handle_location(xml):
    return parser_text(xml, text='got location')
